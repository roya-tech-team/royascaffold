# RoyaScaff Engine 1.3 — Layered SDLC

This folder is a standalone v1.3 reference engine. It turns the v1.3 plan into an operational Markdown-first contract with executable indexing, validation, and exact-reference context building.
...
```bash
node engine-1.3-sdlc/bin/sdlc.js validate <project-root>
node engine-1.3-sdlc/bin/sdlc.js index <project-root>
node engine-1.3-sdlc/bin/sdlc.js context <project-root> <manifest.md> [output.md]
```

The CLI uses only Node built-ins and is tested against the companion `engine-1.3-sdlc-demo/` blueprint.
...
Workflow approvals, semantic review, evidence quality, and reconciliation decisions remain controlled procedures described by the engine. They are not falsely reported as automated checks.
